...
import '../../domain/models/movimiento_cuenta_proveedor.dart';
import '../providers/payables_providers.dart';
import '../widgets/registrar_pago_dialog.dart';

class DetalleCuentaProveedorScreen extends ConsumerWidget {
  const DetalleCuentaProveedorScreen({super.key, required this.proveedorId});

  final String proveedorId;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final estado = ref.watch(detalleCuentaProveedorProvider(proveedorId));
    final controller = ref.read(detalleCuentaProveedorProvider(proveedorId).notifier);
    final detalle = estado.detalle;

    return Scaffold(
      appBar: AppBar(title: Text(detalle?.nombreProveedor ?? 'Cuenta por Pagar')),
      floatingActionButton: detalle == null
          ? null
          : FloatingActionButton.extended(
              key: const Key('registrarPagoBoton'),
              onPressed: () => _registrarPago(context, ref, detalle.saldoActual),
          ...
          : detalle == null
              ...
              : RefreshIndicator(
                  ...
                  child: ListView(
                    ...
                    children: [
                      Text(detalle.rutProveedor ?? 'Sin RUT', style: Theme.of(context).textTheme.bodyMedium),
  ...
  Future<void> _registrarPago(BuildContext context, WidgetRef ref, double saldoActual) async {
    final solicitado = await showDialog<PagoSolicitado>(
      context: context,
      builder: (_) => RegistrarPagoDialog(saldoActual: saldoActual),
    );
    if (solicitado == null) return;
    await ref.read(detalleCuentaProveedorProvider(proveedorId).notifier).registrarPago(
...
class _TarjetaMovimiento extends StatelessWidget {
  ...
  final MovimientoCuentaProveedor movimiento;
