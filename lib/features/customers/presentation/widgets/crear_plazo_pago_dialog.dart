...
/// Alta de un Plazo de Pago del catálogo de Clientes — nombre + lista de
...
class _CrearPlazoPagoDialogState extends ConsumerState<CrearPlazoPagoDialog> {
      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            Text('Cuotas (días desde la Venta)', style: Theme.of(context).textTheme.labelLarge),
  ...
  Future<void> _guardar() async {
    ...
    final exito = await ref.read(plazosPagoProvider.notifier).crear(nombre: nombre, diasCuotas: dias);
    ...
    if (exito) {
      ...
      setState(() {
        _guardando = false;
        _error = ref.read(plazosPagoProvider).error ?? 'No se pudo guardar el Plazo de Pago.';
      });
    }
  }
}
